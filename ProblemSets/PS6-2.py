...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Initially dimension to the size of the measurements plus initial pose estimate
    dim = 2 * (1 + num_landmarks)
    
    # make the constraint information matrix and vector
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0  # initial pose
    Omega.value[1][1] = 1.0  # initial pose

    Xi = matrix()
    Xi.zero(dim, 1)
    Xi.value[0][0] = world_size / 2.0  # initial pose
    Xi.value[1][0] = world_size / 2.0  # initial pose
    ...
    for k in range(len(data)):
        ...
        # expand Omega and Xi to insert the new movement
        l1 = [0, 1] + range(4, dim + 2)
    	Omega = Omega.expand(dim + 2, dim + 2, l1)
    	Xi = Xi.expand(dim + 2, 1, l1, [0])
        ...
        # recompute the deflated Omega and Xi
        l2 = range(2, dim + 2)
        OmegaPrime = Omega.take(l2, l2)
        XiPrime = Xi.take(l2, [0])
        A = Omega.take([0, 1], l2)
