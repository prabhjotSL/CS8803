...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if not OTHER:
        OTHER = [[measurement[0]], [measurement[1]], [measurement[0], measurement[1]]]
        ...
        for i in range(len(theta)):
            start = (theta[i] - meantheta * i)
            if start > 0:
                start = start % pi
            else:
                start = start % pi - pi
            starttheta2 += start
        starttheta2 /= len(theta)
        
        curtheta = starttheta2 + len(theta) * meantheta
        xy_estimate = (center[0] + R * cos(curtheta), center[1] + R * sin(curtheta))        

        print "i, center, R, meantheta, starttheta: ", i, center, R, meantheta, starttheta2
...
test_target = robot(0.5*2.1, 0.5*4.3, 0.5*0.5, 0.5*2*pi / 34.0, 0.5*1.5)
