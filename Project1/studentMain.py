...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if not OTHER:
        ...
        OTHER.append(measurement)
        dist = distance_between(OTHER[1], OTHER[0])
        ang = 0.0
        for i in range(2, len(OTHER)):
            dist += distance_between(OTHER[i], OTHER[i - 1])
            ang += angle_between([OTHER[i-0][0] - OTHER[i-1][0], OTHER[i-0][1] - OTHER[i-1][1]], \
                                 [OTHER[i-1][0] - OTHER[i-2][0], OTHER[i-1][1] - OTHER[i-2][1]])
        meandist = dist / (len(OTHER) - 1)
        meanang = ang / (len(OTHER) - 2)
        print "meandist, meanang: ", meandist, meanang
        heading = atan2(OTHER[i][1] - OTHER[i-1][1], OTHER[i][0] - OTHER[i-1][0]) + meanang
        xy_estimate = (measurement[0] + meandist * cos(heading), measurement[1] + meandist * sin(heading))   
...
def angle_between(u, v):
    return acos((u[0]*v[0] + u[1]*v[1]) / (sqrt(u[0]**2 + u[1]**2) * sqrt(v[0]**2 + v[1]**2)))
